...
export async function deploy({ projectId, serviceName, region, files, progressCallback, skipIamCheck }) {
  ...
  try {
    await ensureApisEnabled(projectId, REQUIRED_APIS_FOR_SOURCE_DEPLOY, progressCallback);
 ...
 * @throws {Error} If deployment fails or required configuration is missing.
 */
export async function deployImage({ projectId, serviceName, region, imageUrl, progressCallback, skipIamCheck }) {
  if (!projectId) {
    const errorMsg = "Error: projectId is required in the configuration object.";
    logAndProgress(errorMsg, progressCallback, 'error');
    process.exit(1);
  ...
  try {
    await ensureApisEnabled(projectId, REQUIRED_APIS_FOR_IMAGE_DEPLOY, progressCallback);
    ...
    const service = await deployToCloudRun(projectId, region, serviceName, imageUrl, progressCallback, skipIamCheck);
